...
class SVM():
    ...
    def fit(self, x_train, y_train):
        ...
        self.b = np.mean(self.y_s - np.dot((self.y_s * self.alpha[self.S]).T, self.rbf(self.x_s, self.x_s)))
        print("=======")
        print("b:")
        print(self.b)

    def predict(self, x_test):
        return(np.sign(np.dot((self.y_s * self.alpha[self.S]).T, self.rbf(self.x_s, x_test)) + self.b))
